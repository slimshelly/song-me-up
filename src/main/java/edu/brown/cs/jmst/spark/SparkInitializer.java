...
public class SparkInitializer {
  ...
  public static void setHandlers(FreeMarkerEngine freeMarker, String root) {
    ...
    // PRE PLAYLIST PAGE
    Spark.get("/form", new PartyFormHandler(), freeMarker); // leads to joe's
                                                            // party form, fill
                                                            // out before
                                                            // creating party
    Spark.get("/host", new HostHandler(), freeMarker); // leads to joe's "you
                                                       // have made a party
                                                       // page" pre-playlist

    // PLAYLIST PAGES (2)
    Spark.get("/admin", new AdminPageHandler(), freeMarker); // leads to joe's
                                                             // own playlist
                                                             // page
    Spark.get("/join", new JoinHandler(), freeMarker); // leads to user's
                                                       // playlist page

    // GENERATE INFO FOR PLAYLIST PAGE
    Spark.post("/suggestions", new SongSuggestor());
    Spark.post("/refresh", new RefreshToken());
    Spark.post("/playlist", new PlaylistHandler()); // reloads party playlist on
                                                    // page reload
